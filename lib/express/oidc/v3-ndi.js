...
function config(app, { showLoginPage, isStateless }) {
  ...
  app.get(`/singpass/v3/userinfo`, async (req, res) => {
    ...
    // Step 4: Encrypt user info with RP encryption key
    ...
    const myInfoFields = await Promise.all(
      scopesArr.map((scope) => userInfoScopeToMyInfoField(persona, scope)),
    )
    ...
    const signingKey = await jose.importJWK(aspSigningKey, 'ES256')
    const jwtData = await new jose.SignJWT(data)
      .setProtectedHeader({
        alg: 'ES256',
      })
      .sign(signingKey)

    const jwe = await new jose.CompactEncrypt(Buffer.from(jwtData))
      .setProtectedHeader(encryptedProtectedHeader)
      .encrypt(await jose.importJWK(rpEncryptionKey, rpEncryptionKey.alg))
...
const userInfoScopeToMyInfoField = (persona, scope) => {
  switch (scope) {
